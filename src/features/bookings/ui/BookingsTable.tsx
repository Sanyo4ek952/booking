...
import { statusClassName, statusLabel } from "../model/status";
  ...
  if (bookings.length === 0) {
    return (
      <EmptyState
        ...
        description="Измените фильтр или добавьте первую бронь через форму слева."
      />
    );
  }

  return (
    <div className="overflow-hidden rounded-2xl border border-sand-200 bg-white shadow-xl shadow-stone-900/7">
          ...
          <tbody className="divide-y divide-sand-100">
            {bookings.map((booking) => (
              <tr key={booking.id} className="align-top hover:bg-sand-50/60">
                ...
                <td className="px-4 py-4 font-medium text-graphite-900">
                  {new Intl.NumberFormat("ru-RU", { style: "currency", currency: "RUB", maximumFractionDigits: 0 }).format(booking.amount)}
                </td>
                <td className="px-4 py-4">
                  <Badge className={statusClassName(booking.status)}>{statusLabel(booking.status)}</Badge>
                </td>
                <td className="max-w-56 px-4 py-4 text-graphite-500">
                  <span className="line-clamp-3">{booking.comment || "Нет"}</span>
                </td>
                <td className="px-4 py-4">
                  <div className="flex justify-end gap-2">
                    ...
                    <Button size="icon" variant="danger" onClick={() => onDelete(booking)} aria-label="Удалить">
